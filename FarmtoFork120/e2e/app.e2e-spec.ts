...
describe('Starting tests for FarmtoFork120', function() {
  ...
  it('website title should be FarmtoFork120', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('FarmtoFork120');
    })
  });

  it('network-name should be farmtofork@1.2.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('farmtofork@1.2.0.bna');
    });
  });

  it('navbar-brand should be FarmtoFork120',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('FarmtoFork120');
    });
  });
