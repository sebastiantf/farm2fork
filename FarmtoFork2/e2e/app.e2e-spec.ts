...
describe('Starting tests for FarmtoFork2', function() {
  ...
  it('website title should be FarmtoFork2', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('FarmtoFork2');
    })
  });
  ...
  it('navbar-brand should be FarmtoFork2',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('FarmtoFork2');
    });
  });
