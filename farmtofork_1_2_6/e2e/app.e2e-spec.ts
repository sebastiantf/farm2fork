...
describe('Starting tests for farmtofork1_2_6', function() {
  ...
  it('website title should be farmtofork1_2_6', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('farmtofork1_2_6');
    })
  });

  it('network-name should be farmtofork@2.0.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('farmtofork@2.0.0.bna');
    });
  });

  it('navbar-brand should be farmtofork1_2_6',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('farmtofork1_2_6');
    });
  });
